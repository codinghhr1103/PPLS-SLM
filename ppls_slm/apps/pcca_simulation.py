...
def main(argv: list[str] | None = None) -> int:
    ...
    out_dir = resolve_path(root, str(exp_cfg.get("output_dir", "output/pcca_simulation")))
    ...
    for trial in range(n_trials):
        ...
        for method, mse, runtime in (("BCD-SLM", bcd_mse, bcd_time), ("EM", em_mse, em_time)):
            rows.append(
                {
                    "trial": trial,
                    "seed": trial_seed,
        ...
        print(f"trial {trial+1:02d}/{n_trials} | BCD-SLM: {bcd_time:.2f}s | EM: {em_time:.2f}s")
    ...
    mse_table = {
        ...
        "bcd_slm": {
            ...
            "Sigma_t": {
                **bcd_agg["mse_Sigma_t"],
                "table_str_x1e2": _table_cell_x1e2(bcd_agg["mse_Sigma_t"]["mean"], bcd_agg["mse_Sigma_t"]["std"]),
            },
        },
        "em": {
            ...
            "Sigma_t": {
                **em_agg["mse_Sigma_t"],
                "table_str_x1e2": _table_cell_x1e2(em_agg["mse_Sigma_t"]["mean"], em_agg["mse_Sigma_t"]["std"]),
            },
        },
    }

    save_json(out_dir / "mse_table.json", mse_table)
    ...
    exp_summary = {
        ...
        "mse": {"bcd_slm": bcd_agg, "em": em_agg},
    }
    save_json(out_dir / "experiment_summary.json", exp_summary)
